...
class Breakpoints():
    ...
    #     index = RSS_table[, 1, drop = TRUE]
    #     break_RSS = sapply(index, function(i) RSS.table[as.character(i),breaks*2] + RSS(i + 1, n))
    #     opt = index[which.min(break.RSS)]
    #     if breaks > 1:
    #         for i in ((breaks:2)*2 - 1):
    #             opt = [(RSS.table[as.character(opt[1]),i], opt]
    #     return(opt)
    ...
    def __init__(self, X, y, h=0.15, breaks=None, data=[]):
        ...
        break_RSS = np.array([Breakpoints.RSS(1,i) for i in index])
